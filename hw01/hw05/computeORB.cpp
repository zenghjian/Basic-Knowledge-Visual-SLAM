...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
  ...
  for (auto &kp : keypoints) {
    ...
    int y=kp.pt.y;
    // get key point
    if(x<half_patch_size || y<half_patch_size || x+half_patch_size>image.cols || y+half_patch_size>image.rows)
        continue;
    // 不符合FAST要求的keypoint直接跳过
    double m01=0,m10=0;
    for (int v  = y - half_patch_size; v < (y + half_patch_size - 1); ++v) {
        for (int u = x - half_patch_size; u < (x + half_patch_size - 1); ++u) {
            m01 += v*image.at<uchar >(v,u);//注意这里的坐标，（y，x）
            m10 += u*image.at<uchar >(v,u);//图像矩中的坐标为图像块的像素坐标
        }
    }
    kp.angle = atan2(m01,m10)*180/pi; // compute kp.angle,注意弧度制转换为角度制
...
void computeAngleMT(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
  ...
  int half_patch_size = 8;
  std::for_each(std::execution::par, keypoints.begin(), keypoints.end(),
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints,
  ...
  for (auto &kp : keypoints) {
    ...
    // vector<bool> 用于存储二进制descriptor
    for (int i = 0; i < 256; i++) {
      // START YOUR CODE HERE (~7 lines)
        double theta = kp.angle*pi/180;

        cv::Point2f p_tmp(std::cos(theta)*ORB_pattern[4*i]    -   std::sin(theta)*ORB_pattern[4*i+1],
                        std::sin(theta)*ORB_pattern[4*i]    +   std::cos(theta)*ORB_pattern[4*i+1]);
        cv::Point2f q_tmp(std::cos(theta)*ORB_pattern[4*i+2]  -   std::sin(theta)*ORB_pattern[4*i+3],
                        std::sin(theta)*ORB_pattern[4*i+2]  +   std::cos(theta)*ORB_pattern[4*i+3]);

        // 根据pattern，使得p,q旋转。再加上keypoint的实际位置以获得相对位置
        cv::Point2f p(kp.pt+p_tmp);
        cv::Point2f q(kp.pt+q_tmp);
        
        if(p.x<0||p.x>image.cols||p.y<0||p.y>image.rows||q.x<0||q.x>image.cols||q.y<0||q.y>image.rows){
            d.clear();
            break;
        }        
        // if kp goes outside, set d.clear()
        //判断像素值大小,bool值或者也可用uchar
    ...
    for (int i = 0; i < desc1.size(); ++i) {
        ...
        for (int j = 0; j < desc2.size(); ++j) {
            ...
            for (int k = 0; k < 256; ++k) {
                ...
                // 关键点: 对选取的两个descriptor逐位做XOR，获取hamming distance
            }
