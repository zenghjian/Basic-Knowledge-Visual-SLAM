        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    // get actual pixel in window                    
                    if (inverse == false) {
                        // Forward Jacobian
                        J(0,0) =-1.0 *  (0.5*(GetPixelValue(img2, u2 + 1, v2) - GetPixelValue(img2, u2 - 1, v2)));
                        J(1,0) =-1.0 *  (0.5*(GetPixelValue(img2, u2, v2 + 1) - GetPixelValue(img2, u2, v2 - 1)));
                    } else {
                        // Inverse Jacobian
                        // NOTE this J does not change when dx, dy is updated, so we can store it and only compute error
                        J(0,0) =-1.0 *  (0.5*(GetPixelValue(img1, u1 + 1, v1) - GetPixelValue(img1, u1 - 1, v1)));
                        J(1,0) =-1.0 *  (0.5*(GetPixelValue(img1, u1, v1 + 1) - GetPixelValue(img1, u1, v1 - 1)));                       
                    }
...
void OpticalFlowMultiLevel(
    ...
    for (int i = 0; i < pyramids; i++) {
        ...
        // imshow("pyr1"+i,pyr1[i]);
        // imshow("pyr2"+i,pyr2[i]);
        // 验证图像缩放
    }
    // 建立4层pyr的图像
    // TODO END YOUR CODE HERE
    ...
    // 这里获取金字塔顶层的keypoint坐标，后续每次迭代的时候要将坐标尺度更新到当前层上
    for(int level = pyramids-1; level >= 0; level--)
    {
        //from coarse to fine 
        //由粗至精， 自顶向下
        //level 越大，图像越小，越coarse
        success.clear();
